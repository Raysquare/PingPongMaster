...
namespace RockVR.Video.Demo
{
    public class VideoCaptureUI : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            ...
            {
                // if (GUI.Button(new Rect(900, Screen.height - 60, 150, 50), "Start Capture"))
                // {
            ...
            {
                if (GUI.Button(new Rect(900, Screen.height - 60, 150, 50), "Stop Capture"))
                {
                    VideoCaptureCtrl.instance.StopCapture();
                }
                // if (GUI.Button(new Rect(950, Screen.height - 60, 150, 50), "Pause Capture"))
                // {
            ...
            else if (VideoCaptureCtrl.instance.status == VideoCaptureCtrl.StatusType.PAUSED)
            {
                ...
                if (GUI.Button(new Rect(950, Screen.height - 60, 150, 50), "Continue Capture"))
                {
                    VideoCaptureCtrl.instance.ToggleCapture();
                }
            }
            ...
            {
                if (GUI.Button(new Rect(900, Screen.height - 60, 150, 50), "Processing"))
                {
            ...
            else if (VideoCaptureCtrl.instance.status == VideoCaptureCtrl.StatusType.FINISH)
            {
                if (!isPlayVideo)
                {
                    if (GUI.Button(new Rect(900, Screen.height - 60, 150, 50), "View Video"))
                    {
                ...
                {
                    if (GUI.Button(new Rect(900, Screen.height - 60, 150, 50), "Next Video"))
                    {
                        // Turn to next video.
